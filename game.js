document.addEventListener("DOMContentLoaded", () => {
    ...
    function create() {
        ...
        // player1 text
        this.add.text(100, 80, player1Text, {
            fontSize: '16px',
            fill: '#ffffff',
            shadow: {
                offsetX: 2,
        ...
        // player2 text
        this.add.text(600, 80, player2Text, {
            fontSize: '16px',
            fill: '#ffffff',
            shadow: {
                offsetX: 2,
        ...
        if (this.sys.game.device.os.android || this.sys.game.device.os.iOS) {
            ...
            // Left move
            const leftZone = this.add.zone(0, h - 100, w / 3, 100).setOrigin(0).setInteractive();
            leftZone.on('pointerdown', () => moveLeft = true);
            leftZone.on('pointerup', () => moveLeft = false);
            leftZone.on('pointerout', () => moveLeft = false);

            // Right move
            const rightZone = this.add.zone(w / 3, h - 100, w / 3, 100).setOrigin(0).setInteractive();
            rightZone.on('pointerdown', () => moveRight = true);
            rightZone.on('pointerup', () => moveRight = false);
            rightZone.on('pointerout', () => moveRight = false);

            // Punch
            const punchZone = this.add.zone((2 * w) / 3, h - 100, w / 3, 100).setOrigin(0).setInteractive();
            punchZone.on('pointerdown', () => isPunching = true);
            punchZone.on('pointerup', () => isPunching = false);
            punchZone.on('pointerout', () => isPunching = false);

            // Optional UI visuals
            this.add.text(w / 6, h - 80, '←', { fontSize: '40px', color: '#fff' }).setOrigin(0.5);
            this.add.text(w / 2, h - 80, '→', { fontSize: '40px', color: '#fff' }).setOrigin(0.5);
            this.add.text((5 * w) / 6, h - 80, '🥊', { fontSize: '40px', color: '#fff' }).setOrigin(0.5);
        }
    ...
    function update() {
        ...
        if (Phaser.Input.Keyboard.JustDown(this.cursors.space) || isPunching) {
